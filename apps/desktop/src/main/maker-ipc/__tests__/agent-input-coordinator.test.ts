...
describe('AgentInputCoordinator Orca priority queue transactions', () => {
  it.each(['dsh-prompt-unconfirmed', 'dsh-prompt-timeout'] as const)(
    'retains %s DSH input and attachments without allowing explicit replay',
    async (reason) => {
      ...
      await flush();
      h.coordinator.onTurnEvent(sid, 'error', 'DSH result uncertain', { reason });
