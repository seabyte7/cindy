...
describe('ProviderConnectionDialog accessibility', () => {
  ...
  it('shows DSH as a dedicated fourth Runtime tab and requires a replacement key when its endpoint changes', async () => {
    const initial: CustomProviderConfig = {
      id: 'dsh-provider',
      name: 'DSH provider',
      auth: { method: 'apiKey' },
    ...
    expect(
      screen.getByRole('tab', { name: 'settings.providers.custom.dsh.label' }).getAttribute('aria-selected'),
    ).toBe('true');
  ...
  it('shows only Main-projected DSH status and retries through the fixed runtime action', async () => {
    const getDshRuntimeStatus = vi.fn(async () => ({
      revision: 1,
      configuration: { status: 'ready' as const, providerName: 'DeepSeek' },
      registration: 'task-factory-ready' as const,
      activeTaskCount: 0,
      evidence: {
    ...
    const retryDshRuntimeRegistration = vi.fn(async () => await getDshRuntimeStatus());
    Object.defineProperty(window, 'electronAPI', {
      configurable: true,
      value: {
        maker: {
    ...
    const user = userEvent.setup();
    render(<CustomProviderDialog initial={initial} onSaved={vi.fn()} onClose={vi.fn()} />);
    await screen.findByText('settings.providers.custom.dsh.runtimeStatus.configuration.ready');
    expect(screen.getByText('settings.providers.custom.dsh.runtimeStatus.registration.task-factory-ready')).toBeTruthy();
    expect(screen.getByText('settings.providers.custom.dsh.runtimeStatus.evidence')).toBeTruthy();
    await user.click(screen.getByRole('button', { name: 'settings.providers.custom.dsh.runtimeStatus.retry' }));
    await waitFor(() => expect(retryDshRuntimeRegistration).toHaveBeenCalledOnce());
  });
